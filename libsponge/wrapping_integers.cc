...
inline uint64_t dis(uint64_t a, uint64_t b) { return (a > b) ? (a - b) : (b - a); };
...
uint64_t unwrap(WrappingInt32 n, WrappingInt32 isn, uint64_t checkpoint) {
    ...
    uint64_t min_gap = std::min(std::min(dis(cand1, checkpoint), dis(cand2, checkpoint)), dis(cand3, checkpoint));
